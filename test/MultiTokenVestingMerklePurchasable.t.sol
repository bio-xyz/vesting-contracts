...
import { MultiTokenVestingMerklePurchasable } from "../contracts/vesting/vestingMultiple/MultiTokenVestingMerklePurchasable.sol";
import { TokenVestingMerklePurchasable } from "../contracts/vesting/TokenVestingMerklePurchasable.sol";
import { MultiTokenVesting } from "../contracts/vesting/vestingMultiple/MultiTokenVesting.sol";
...
contract TokenVestingV2MerklePurchasableTest is Test {
    ...
    function setUp() public {
        ...
        tokenVesting2 = new MultiTokenVestingMerklePurchasable(
            IERC20Metadata(token), "Virtual Test Token2", "vTT", paymentReceiver, vestingCreator, vTokenCost, merkleRoot1, address(tokenVesting)
        );

        tokenVesting3 = new TokenVestingMerklePurchasable(
            IERC20Metadata(token), "Virtual Test Token2", "vTT", paymentReceiver, vestingCreator, vTokenCost, merkleRoot1
        );
    ...
    function testCanClaimSchedule() public {
        ...
        vm.startPrank(alice);
        tokenVesting.claimSchedule{ value: purchasePrice }(aliceProof, baseTime, cliff, duration, 1, true, claimableAmount);
        vm.stopPrank();
    ...
    function testCantClaimTwiceAcrossContract() public {
        ...
        tokenVesting.claimSchedule{ value: purchasePrice }(aliceProof, baseTime, cliff, duration, 1, true, claimableAmount);
        vm.expectRevert(MultiTokenVestingMerklePurchasable.AlreadyClaimed.selector);
        tokenVesting2.claimSchedule{ value: purchasePrice }(aliceProof, baseTime, cliff, duration, 1, true, claimableAmount);
        vm.stopPrank();
